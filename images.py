...
class Image2(mn.Scene):
    def construct(self):
        line = (
            r"$X_n \stackrel{L^p}{\to} X \implies \dots$	$$X_n \stackrel{p}{\to} X$$			"
        )
        group = aw.generate_vgroup(line)
        self.add(group)
...
class Image83(mn.Scene):
    def construct(self):
        line = (
            r"Für $H \leq G$ ist die Ordnung von [...] ein Teiler von [...]	$$H, |G|$$			"
        )
        group = aw.generate_vgroup(line)
        self.add(group)
...
class Image93(mn.Scene):
    def construct(self):
        line = (
            r"$N \trianglelefteq G$	$$gN = Ng \ \forall g \in G$$	Normalteiler von G		"
        )
        group = aw.generate_vgroup(line)
        self.add(group)
...
class Image107(mn.Scene):
    def construct(self):
        line = (
            r"Polish space	separable, completely metrizable topological space&nbsp;			"
        )
        group = aw.generate_vgroup(line)
        self.add(group)
...
class Image117(mn.Scene):
    def construct(self):
        line = (
            r"supermartingale	$$\mathbb{E}(X_n | \mathcal{F}_{n-1}) \leq X_{n-1}$$			"
        )
        group = aw.generate_vgroup(line)
        self.add(group)
...
class Image128(mn.Scene):
    def construct(self):
        line = (
            r"$W^\perp$	$$\{x \in H | \langle w, x \rangle = 0 \ \forall w \in W\}$$			"
        )
        group = aw.generate_vgroup(line)
        self.add(group)
...
class Image173(mn.Scene):
    def construct(self):
        line = (
            r"freie Gruppe	$$F_S := \{ \ \text{Wörter mit den Variablen aus} \ S \ \}$$			"
        )
        group = aw.generate_vgroup(line)
        self.add(group)
...
class Image296(mn.Scene):
    def construct(self):
        line = (
            r"homogeneous Markov chain	$$\bar{\mathbb{P}}_l = \bar{\mathbb{P}}_k$$			"
        )
        group = aw.generate_vgroup(line)
        self.add(group)
...
class Image305(mn.Scene):
    def construct(self):
        line = (
            r"closed communicating class	$$i \in C \wedge i \to j \implies j \in C$$			"
        )
        group = aw.generate_vgroup(line)
        self.add(group)
...
class Image345(mn.Scene):
    def construct(self):
        line = (
            r"$b$ ist assoziiert zu $a$&nbsp;	$$\exists c \in R^{\times}: a = bc$$			"
        )
        group = aw.generate_vgroup(line)
        self.add(group)
